...
const Services = () => {
    ...
    const servicesContent = {
        ...
        title: 'Servicios Contables y de Consultoría',
        btn_text: 'TODOS LOS SERVICIOS',
        btn_link: '/servicios'
    }
    return (
                    ...
                    {servicesItem?.map((data, id) => (
                        <div className="col-xl-3 col-lg-4 col-md-6 mt-25" key={id}>
                            <div className="services__one-item">
                                ...
                                <Link className="simple-btn-2" href={`/services/${data.id}`}>Leer Más<i className="far fa-chevron-double-right"></i></Link>
                            </div>
                        </div>
                    ))}
                </div>
